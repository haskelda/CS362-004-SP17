...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      cardEffect_Adventurer(state, handPos);
...
// NOTE: this function was edited (fixed) for assignment-5;  discardCard() and its argument handPos were added
int cardEffect_Adventurer(struct gameState *state, int handPos)
{
  int currentPlayer = whoseTurn(state);
  ...
  discardCard(handPos, currentPlayer, state, 0); // place adventurer into playedCards
  return 0;
}
